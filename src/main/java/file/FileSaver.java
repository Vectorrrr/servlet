 ...
 * the file to disk, as well as receipt of all Existing
 * files with a specific filter
 * @author Ivan Gladush
 * @since 15.04.16.
 */
public class FileSaver implements AutoCloseable {
    ...
    //todo maybe it's worth to make one
    private static final String EXCEPTION_CREATE_FILE = "I don't create a new file for download this document";
    private static final String EXCEPTION_CREATE_NEW_FILE = "Failed to create a new file to save resources";
    ...
    private File createNewFile(String fileName) throws IOException {
        ...
        if (!f.createNewFile()) {
            logger.warn(EXCEPTION_CREATE_NEW_FILE);
